...
*/
template <typename CharT> Password<CharT>::~Password() { std::free(m_seed); }
...
template <typename CharT>
void Password<CharT>::setSeed(const uchar *t_seed,
                              const std::size_t t_seedlen) {
    std::free(m_seed);
...
template <typename CharT> const uchar *Password<CharT>::getSeed() const {
    return this->m_seed;
}
